...
const postSaque = async (req: Request, res: Response) => {
  const response = await contaService.postSaque(req.body.CodCliente, req.body.Valor);
...
const postDeposito = async (req: Request, res: Response) => {
  const response = await contaService.postDeposito(req.body.CodCliente, req.body.Valor);
  if (typeof response === 'string') return res.status(404).json({ message: response });
...
const getContaByClient = async (req: Request, res: Response) => {
  const cliente = await contaService.getContaByClient(parseInt(req.params.CodCliente, 10));
